...
export default function FriendsScreen() {
  ...
  // --- STATI COLLEZIONE ---
  const [selectedFriend, setSelectedFriend] = useState<any>(null); // Per vedere la collezione
  const [friendCats, setFriendCats] = useState<any[]>([]);
  const [loadingCats, setLoadingCats] = useState(false);
  const [removing, setRemoving] = useState(false);

  // --- STATI CHAT ---
  const [chatFriend, setChatFriend] = useState<any>(null); // Per la chat aperta
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        {
          ...
          filter: `receiver_id=eq.${myId}`, // Ascolta messaggi per ME
        },
        (payload) => {
           // Se il messaggio arriva dall'amico con cui sto parlando, aggiungilo
  ...
  const sendMessage = async () => {
    ...
    setNewMessage(""); // Pulisci subito input

    // Inserimento ottimistico (lo mostro subito)
    const optimisticMsg = {
        id: Date.now(),
        sender_id: myId,
  ...
  return (
    <div className="h-full w-full overflow-y-auto text-black pb-32">
            ...
            <div className="space-y-2.5">
                {friends.map((f) => (
                    <div key={f.user_id} className="w-full bg-white py-3 px-4 rounded-2xl shadow-sm border-b-4 border-gray-100 flex items-center gap-3">
                        ...
                        {/* AZIONI: CHAT e COLLEZIONE */}
                        <div className="flex gap-2">
                            <button onClick={() => openChat(f)} className="w-10 h-10 bg-blue-100 hover:bg-blue-200 text-blue-600 rounded-xl flex items-center justify-center active:scale-90 transition shadow-sm border-b-2 border-blue-200">
                                💬
                            </button>
                            <button onClick={() => openFriendCollection(f)} className="w-10 h-10 bg-yellow-100 hover:bg-yellow-200 text-yellow-600 rounded-xl flex items-center justify-center active:scale-90 transition shadow-sm border-b-2 border-yellow-200">
                                🃏
                            </button>
                        </div>
                    </div>
      ...
      <AnimatePresence>
        {chatFriend && (
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="fixed inset-0 z-[250] bg-gray-50 flex flex-col">
                
                {/* HEADER CHAT */}
                <div className="bg-white px-4 py-4 pt-12 shadow-sm flex items-center gap-3 border-b border-gray-100 z-10">
                    <button onClick={() => setChatFriend(null)} className="w-10 h-10 bg-gray-100 rounded-full font-bold text-xl active:scale-90 transition">←</button>
                    <div className="flex-1">
                        <div className="font-black text-lg leading-none">{chatFriend.email?.split('@')[0]}</div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-[#f0f2f5]">
                    {messages.length === 0 && <div className="text-center text-gray-400 text-sm mt-10 font-bold">Inizia la conversazione! 👋</div>}
                    
                    {messages.map((msg) => {
                        const isMe = msg.sender_id === myId;
                        return (
                            <div key={msg.id} className={`flex w-full ${isMe ? "justify-end" : "justify-start"}`}>
                                <div className={`max-w-[75%] px-4 py-2.5 rounded-2xl text-sm font-bold shadow-sm break-words ${isMe ? "bg-black text-white rounded-br-none" : "bg-white text-gray-800 rounded-bl-none border border-gray-200"}`}>
                                    {msg.content}
                                </div>
                            </div>
                        );
                ...
                {/* INPUT CHAT */}
                <div className="p-3 bg-white border-t border-gray-200 pb-8">
                    <form onSubmit={(e) => { e.preventDefault(); sendMessage(); }} className="flex gap-2">
                        ...
                        />
                        <button type="submit" disabled={!newMessage.trim()} className="w-12 h-12 bg-blue-500 text-white rounded-full flex items-center justify-center font-black text-lg shadow-lg active:scale-90 transition disabled:opacity-50 disabled:scale-100">
                            ➤
                        </button>
                    </form>
                </div>
